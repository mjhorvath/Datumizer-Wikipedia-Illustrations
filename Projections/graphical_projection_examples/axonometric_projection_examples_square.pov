...
#declare proj_type	= 3;		// integer: type of projection being demonstrated; 0 = multiview, 1 = isometric, 2 = oblique, 3 = perspective
#declare floor_drop	= 4;		// float: distance of floor from center
#declare trans_zamt	= 2;		// float
#declare cam_view	= 0;		// integer: camera perspective; 0 = perspective pictorial, 1 = orthographic top, 2 = orthographic front
#declare pane_size	= <4,4,0>;	// vector
#declare base_size	= <4,0,4>;	// vector
#declare cube_scale	= 1;		// float
...
#switch (proj_type)
	...
	#case (2)
		...
		{
			scale cube_scale
			translate -x * sqrt(2)*2
			translate -y * sqrt(2)*2
			translate +z * trans_zamt
		}
		#local pane_tran = transform
		{
...
Axes_Macro
(
	32,		// Axes_axesSize,	The distance from the origin to one of the grid's edges.		(float)
