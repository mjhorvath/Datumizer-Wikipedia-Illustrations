...
#declare CSolid_Grid = object
{
	SGrid_Cube_Macro
	(
		<6,6,6,>,		// The number of sections along each axis.	(integer vector)
		<1,1,1,>,		// The width of the cuboid along each axis.	(float vector)
		<0,0,0,>,		// The coordinates of the bottom corner.	(float vector)
		0.01,			// The thickness of the grid lines.		(float)
		off,			// Determines whether the divisions are offset by half the amount. Sometimes necessary when doing cut-aways.	(boolian)
		off,			// Determines whether borders are created at each end of the object. Ignored if the offset is turned on. 	(boolian)
	)
}
...
//	box {CSolid_sRadius/2, CSolid_sRadius}
	...
	{
		4, <0, 0,>, <1, 0,>, <1, 1,>, <0, 1,>
		pigment {image_map {png "CSolid_Hue_Short_Below.png"}}
		translate	<-0.5, -0.5>
		scale		8/3
		rotate		+z * 90
		rotate		+x * 90
	...
	{
		4, <0, 0,>, <1, 0,>, <1, 1,>, <0, 1,>
		pigment {image_map {png "CSolid_Lightness_Short_Below.png"}}
		translate	<-0.5, -0.5>
		scale		8/3
		rotate		+z * 180
		rotate		+y * 90
	...
	{
		4, <0, 0,>, <1, 0,>, <1, 1,>, <0, 1,>
		pigment {image_map {png "CSolid_Saturation_Short_Below.png"}}
		translate	<-0.5, -0.5>
		scale		8/3
		rotate		+z * 90
		rotate		+x * 90
