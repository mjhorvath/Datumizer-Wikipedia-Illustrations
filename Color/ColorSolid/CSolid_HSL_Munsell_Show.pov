...
#declare CSolid_Grid = object
{
	SGrid_Munsell_Macro
	(
		6,			// SGrid_Munsell_radii,			// The number of radial divisions.	(integer)
		12,			// SGrid_Munsell_longt,			// The number of longitudinal divisions.	(integer)
		6,			// SGrid_Munsell_lattt,			// The number of lattitudinal divisions.	(integer)
		1,			// SGrid_Munsell_radius,		// The radius of the sphere.	(float)
		0,			// SGrid_Munsell_center,		// The center coordinates of the sphere. (vector)
		0.01,			// SGrid_Munsell_thickness,		// The thickness of the grid lines. (float)
		off,			// SGrid_Munsell_offset,		// Determines whether the divisions are offset by half the amount (sometimes necessary when doing cut-aways at intervals matching the grid's divisions).	(boolian)
		off,			// SGrid_Munsell_endcap,		// Determines whether borders are created at each end of the object. Ignored if the offset is turned on. 	(boolian)
	)
}
...
difference
{
	...
	texture
	{
		pigment {CSolid_HSLMunsell_Pigment}
	}
}
