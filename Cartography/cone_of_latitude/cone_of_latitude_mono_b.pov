// The Earth texture is spherically UV mapped to the ellipsoid using map_type 1, and the graticule is based on geodetic latitude. The map_type 1 was never intended to be used with ellipsoids, however, so it needs to be replaced with something better.
...
{
	0,0.316227766016838,y*sphere_scale,0.724476056480703				// values copied directly from geogebra
}
...
#declare earth_socket = difference
{
	object {earth_surface}
	object {cone_of_intersection}
	pigment {color srgb x/2}
}

#declare spheredrop_time = get_time(1/2, 1);
object {earth_plug		translate +y * spheredrop_time * 1/2}
...
{
	0.632455532033677,0.005				// values copied directly from geogebra
	translate y * 0.547722557505168				// values copied directly from geogebra
...
#declare plane_wedge = object
{
	...
	intersection
	{
		...
		translate +y * spheredrop_time * 1/2
	}
#end
